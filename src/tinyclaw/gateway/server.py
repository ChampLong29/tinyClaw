...
class GatewayServer:
    ...
    def __init__(
        ...
        session_resolver_fn: Any = None,  # (agent_id, channel, account, peer, user) -> sk
        host: str = "localhost",
        port: int = 8765,
    ) -> None:
        self._mgr = mgr
        ...
        self._session_resolver = session_resolver_fn
        self._host = host
        self._port = port
    ...
    async def _m_send(self, p: dict) -> dict:
        ...
        account_id = p.get("account_id", "gateway")
        pid = p.get("peer_id", "ws-client")
        platform_user_id = p.get("platform_user_id") or p.get("user_id") or pid
        if p.get("agent_id"):
